...
if (isset($_POST['submit'])) {
  ...
  $carModel = $mysqli->real_escape_string($carModel);
  $year = $mysqli->real_escape_string($year);
  $engineCC = $mysqli->real_escape_string($engineCC);
  $rtPrice = $mysqli->real_escape_string($rtPrice);
  $insComp = $mysqli->real_escape_string($insComp);
  $sumCovered = $mysqli->real_escape_string($sumCovered);
  $ncd = $mysqli->real_escape_string($ncd);
  $expiryDate = $mysqli->real_escape_string($expiryDate);
  $basicContrb = $mysqli->real_escape_string($basicContrb);
  $perils = $mysqli->real_escape_string($perils);
  $extraCovrg = $mysqli->real_escape_string($extraCovrg);
  $sst = $mysqli->real_escape_string($sst);
  $stampDuty = $mysqli->real_escape_string($stampDuty);

  $insert = $mysqli->query("INSERT into quotations(vkey, datetime, carModel, year, engineCC, rtPrice, insComp, sumCovered, ncd, expiryDate, basicContrb, perils, extraCovrg, sst, stampDuty ) VALUES ('$vkey',NOW(),'$carModel','$year','$engineCC','$rtPrice','$insComp','$sumCovered','$ncd','$expiryDate','$basicContrb', '$perils','$extraCovrg','$sst','$stampDuty')");
  if ($insert) {

    // Update status in users db
    $setOrderCreated = $mysqli->query('UPDATE `users` SET `status`= 1 WHERE `vkey` ="' . $vkey . '"');

    //Send email
    $from = "yana@instaroadtax.my";
    $to = $email;
    $subject = "⚡InstaRoadTax - Senarai sebutharga insuran untuk " . $type . " " . $plate;
...
<table style="font-family:\'Open Sans\',sans-serif;" role="presentation" cellpadding="0" cellspacing="0" width="100%" border="0">
  ...
  <div style="color: #444444; line-height: 200%; text-align: center; word-wrap: break-word;">
    <p style="line-height: 200%; font-size: 14px;"><span style="font-size: 22px; line-height: 44px;">Salam,</span><br />Anda telah memohon sebutharga insuran untuk '.strtolower($type) .' berpendaftaran ' . $plate . '  melalui InstaRoadTax.my!</p>
